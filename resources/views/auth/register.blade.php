...
// Página de Registro de Usuário e Coleta de Dados para LXPay.
...
<body class="flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-md bg-gray-900 p-8 rounded-xl shadow-2xl border border-gray-700">
        <h1 class="text-3xl font-extrabold text-white text-center mb-2">Crie sua Conta Escalefy</h1>
        <p class="text-gray-400 text-center mb-8 text-sm">Rastreamento e Automação de Meta Ads em segundos.</p>
        ...
        <div class="relative flex justify-center text-xs uppercase mb-6">
            <span class="px-2 text-gray-500 bg-gray-900 z-10">ou registre-se com e-mail</span>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            @if ($errors->any())
                <div class="bg-red-900/50 p-3 rounded-lg text-red-300 text-sm border border-red-700">
                    Houve um erro no formulário. Verifique os campos e tente novamente.
                </div>
            @endif
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required autofocus class="mt-1 block w-full bg-gray-800 border-gray-700 text-white rounded-lg p-2.5 focus:ring-escalefy focus:border-escalefy" autocomplete="name">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email') }}" required class="mt-1 block w-full bg-gray-800 border-gray-700 text-white rounded-lg p-2.5 focus:ring-escalefy focus:border-escalefy" autocomplete="email">
                @error('email') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
            </div>

            <!-- CPF/CNPJ (Requisito LXPay) -->
            <div>
                <label for="document" class="block text-sm font-medium text-gray-400">CPF ou CNPJ (Para Pagamento PIX)</label>
                <input type="text" name="document" id="document" value="{{ old('document') }}" required class="mt-1 block w-full bg-gray-800 border-gray-700 text-white rounded-lg p-2.5 focus:ring-escalefy focus:border-escalefy" placeholder="Ex: 123.456.789-00 ou 11.222.333/0001-44">
            ...
            <div>
                ...
                <input type="password" name="password" id="password" required class="mt-1 block w-full bg-gray-800 border-gray-700 text-white rounded-lg p-2.5 focus:ring-escalefy focus:border-escalefy" autocomplete="new-password">
                @error('password') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
            </div>

            <!-- Confirmação de Senha -->
            <div>
                <label for="password_confirmation" class="block text-sm font-medium text-gray-400">Confirmar Senha</label>
                <input type="password" name="password_confirmation" id="password_confirmation" required class="mt-1 block w-full bg-gray-800 border-gray-700 text-white rounded-lg p-2.5 focus:ring-escalefy focus:border-escalefy" autocomplete="new-password">
            </div>
            ...
            <button type="submit" class="w-full bg-escalefy text-gray-900 font-bold py-3 rounded-lg hover:bg-escalefy/90 transition duration-150">
                Registrar e Escolher Plano
            </button>
            
            <div class="text-center text-sm mt-4">
                <a href="{{ route('login') }}" class="text-escalefy hover:underline">Já tem uma conta? Faça login.</a>
            </div>
        </form>
    </div>
</body>
